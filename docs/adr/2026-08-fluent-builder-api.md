...
Setting up test entities with specific attributes and area assignments was verbose, requiring two separate calls:
...
- `in_area(area: str, labels: Optional[list[str]] = None)` — assigns area and optionally labels
- `with_labels(labels: list[str])` — assigns labels independently
...
The method `given_entity_has()` remains internally for rollback operations but is no longer part of the public API.
...
- **Stateless builder**: The builder delegates all operations to the `HomeAssistant` client; it holds no state beyond the current attributes dict
...
- **Attribute merging**: Successive `with_attributes()` calls merge into the builder's internal dict, then apply via `set_state()`
...
- **`given_entity_has()` retained internally**: Allows rollback logic to reuse existing code paths
