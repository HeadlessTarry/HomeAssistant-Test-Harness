...
- **Child ticket**: an issue linked to the map as a GitHub sub-issue
  ...
  add the child to a task list in the map body and put `Part of #<map>`
  at the top of the child body. Labels: `wayfinder:<type>`
  (`research`/`prototype`/`grilling`/`task`). Once claimed, the ticket
...
- **Blocking**: GitHub's **native issue dependencies** — the canonical,
  ...
  Where dependencies aren't available, fall back to a `Blocked by: #<n>, #<n>` line
